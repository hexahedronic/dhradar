...
local function dhradar_LoadAllBeacons()
	...
	local files, folders = file.Find("lua/" .. path .. "*.lua", "GAME")
	...
	if (dhradar_dat.STOR_lastsize ~= ui_size)
	   ...
	   then
		...
		for i=1, PARAM_HEIGHTDRAW_CIRCLERES do
			local deg = (i * 360) / PARAM_HEIGHTDRAW_CIRCLERES
			local rad = math.rad(deg)

			dhradar_dat.STOR_circlebuffer[i] = {}
			dhradar_dat.STOR_circlebuffer[i].x = ui_xCenter + math.cos(rad) * ui_size * 0.5 * 0.99
			dhradar_dat.STOR_circlebuffer[i].y = ui_yCenter + math.sin(rad) * ui_size * 0.5 * 0.99
			dhradar_dat.STOR_circlebuffer[i].u =  (math.cos(math.rad(-orient + deg)) + 1) * 0.5 + oscr_xpan
			dhradar_dat.STOR_circlebuffer[i].v = -(math.sin(math.rad(-orient + deg)) + 1) * 0.5 + oscr_ypan
		end
...
local function dhradar_InitializeRenderTarget()
	...
	print("DepthHUD Radar: Attempting to create RenderTarget...")
	...
	if not dhradar_dat.RT then
		...
		print("DepthHUD Radar Error: Failed to create RenderTarget!")
...
local filter = bit.bor(CONTENTS_TESTFOGVOLUME, CONTENTS_HITBOX, CONTENTS_TRANSLUCENT, CONTENTS_DETAIL, CONTENTS_DEBRIS, CONTENTS_GRATE, CONTENTS_PLAYERCLIP)
local function dhradar_FindHeights()
	...
	if (RealTime() < (TIME_HEIGHTDRAW_LastPrecise + TIME_HEIGHTDRAW_PRECISEDELAY)
		or (not (dhradar_dat.ui_scale >= PARAM_HEIGHTDRAW_HIGHSCALE_CAP)
			and PARAM_HEIGHTDRAW_Step <= PARAM_HEIGHTDRAW_STEP_MIN)
		or ((dhradar_dat.ui_scale >= PARAM_HEIGHTDRAW_HIGHSCALE_CAP)
			and (PARAM_HEIGHTDRAW_Step <= PARAM_HEIGHTDRAW_STEP_MIN_IF_HIGHSCALE)))
		and dhradar_dat.STOR_lastdhradar_Pos == dhradar_Pos()
		and dhradar_dat.STOR_lastscale == dhradar_dat.ui_scale
	then return end
...
local function dhradar_DrawHeights()
	...
	if dhradar_dat.STOR_lastopacity ~= ui_overallopacity or dhradar_dat.STOR_lastheightopacity ~= ui_heightopacity then
		dhradar_dat.MATERIAL:SetFloat("$alpha", ui_overallopacity * ui_heightopacity)
	end
...
function dhradar_CalcGetPolar(vectorWorldPos)
	...
	local isup			= 0
	local hightdef 	= (1 - PARAM_HEIGHT_PLAYERHEIGHTDEF) * PARAM_HEIGHT_TOLERANCE

	if dhradar_dat.STOR_RadarTrashVector_Position.z >= hightdef then
		isup = 1
	elseif dhradar_dat.STOR_RadarTrashVector_Position.z < -hightdef then
		isup = -1
	end
...
function dhradar_StringNiceNameTransform(stringInput)
	local stringParts = stringInput:Split("_")
	local stringOutput = ""
...
function dhradarHudPaint()
	...
	if GetConVarNumber("dhradar_forwardexplore") > 0 then
		...
		dhradar_SetPos(EyePos() + Angle(0, dhradar_Angles().y, 0):Forward() * math.Round(dhradar_dat.ui_forwardexplore[2]*100)*0.01 * dhradar_dat.ui_scale * PARAM_SCALEBASE + Vector(0,0,1) * math.Round(dhradar_dat.ui_upexplore[2]*100)*0.01 * dhradar_dat.ui_scale * PARAM_SCALEBASE)
